  ...
  function(
    ...
    $scope.showConfirm = function(data) {
      ...
      var confirm = $mdDialog
        ...
        .textContent(
          data.my_handle + " wants to connect.Do you want to Connect?"
        )
        .ariaLabel("Lucky day")
        .ok("Ok")
        .cancel("No");
      ...
      $mdDialog.show(confirm).then(
        function() {
          ...
          $http({
            method: "POST",
            url: "http://" + url + "/friend_request/confirmed",
            data //, headers:{
            //'Content-Type': 'application/json'
            //}
          });
    ...
    socket.on("group", function(data) {
      ...
      if (data.split("#*@")[1] != $scope.user) {
        ...
        document.getElementById("group").scrollTop = document.getElementById(
          "group"
        ).scrollHeight;
      }
    });
    ...
    socket.on("private message", function(data) {
      ...
      document.getElementById(
        data.split("#*@")[2]
      ).scrollTop = document.getElementById(data.split("#*@")[2]).scrollHeight;
    });
    ...
    $scope.send_message = function(chat, message) {
      ...
      socket.emit(
        "private message",
        chat + "#*@" + message + "#*@" + $scope.user + "#*@" + getDate()
      );
    ...
    $scope.chat_popup = function(chat_friend) {
      ...
      div = document.createElement("div");
      div.innerHTML =
        '<div class="popup-box popup-box-on chat-popup" id="' +
        chat_friend +
        '01">\
                        <div class="popup-head">\
                        <div class="popup-head-left pull-left"><img alt="pic">' +
        chat_friend +
        '</div>\
                        ...
                        <button  ng-click="close_chat(\'' +
        chat_friend +
        '\')" class="chat-header-button pull-right" type="button">  <i class="glyphicon glyphicon-remove"></i></button>\
                        </div>\
                        </div>\
        ...
        '" >\
                        ...
                        <textarea id="status_message" placeholder="Type a message..." rows="10" cols="40" ng-model="message" my-enter="send_message(\'' +
        chat_friend +
        '\',\'{{message}}\')"></textarea>\
                        ...
                        <button class="bg_none pull-right" ng-click="send_message(' +
        chat_friend +
        ',message)"><i class="glyphicon  glyphicon-thumbs-up"></i> </button>\
                        </div>\
                        </div>\
                        </div>';
      $compile(div)($scope);
      ...
      if (popups.length > 1) {
        document.getElementById(
          ...
          .getElementById(popups[popups.length - 2] + "01")
          .className.replace(/(?:^|\s)popup-box-on(?!\S)/g, "");
      }
      var body = document.getElementsByTagName("body")[0];
      body.appendChild(div);
      if (localStorage.getItem(chat_friend) !== null) {
        $scope.messages[chat_friend] = JSON.parse(
          localStorage.getItem(chat_friend)
        );
      }
...
<div class="direct-chat-info clearfix">\
<span class="direct-chat-name pull-left">' +
              $scope.messages[chat_friend][i].sender +
              '</span>\
<span class="direct-chat-timestamp pull-right">' +
              $scope.messages[chat_friend][i].date +
              '</span>\
</div>\
<img class="direct-chat-img" src="" alt="message user image">\
<div class="direct-chat-text">' +
            ...
            document.getElementById(
              chat_friend
            ).scrollTop = document.getElementById(chat_friend).scrollHeight;
          } else {
            div = document.createElement("div");
            div.innerHTML =
              '<div class="direct-chat-msg right">\
    ...
    $scope.close_chat = function(chat_friend) {
      ...
      for (var iii = 0; iii < popups.length; iii++) {
        if (chat_friend == popups[iii]) {
          ...
          var chat_box = document.getElementById(
            popups[popups.length - 1] + "01"
          );
...
app.controller("registerController", [
  ...
  "$http",
  "$state",
  function($scope, encrypt, $http, $state) {
    url = location.host;
